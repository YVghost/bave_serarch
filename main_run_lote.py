...
# - q1 (barata) con count bajo
# - q2 (carrera con sinónimos) SOLO si q1 no dio señal suficiente
# - Variantes 2/3 SOLO si sigue muy bajo
# - Reinicio: salta URLs ya llenas y también "SR" para no re-gastar requests
...
def process_row(
    ...
    cache: dict,
    per_query_count_q1: int = 5,     # MÁS BARATO: q1 con menos resultados
    per_query_count_q2: int = 8,     # q2 un poco más para precisión
    ...
    def fetch_query(q: str, count: int) -> list[dict]:
        ...
        results = brave_search_with_rotation(
    ...
    # ---------------- FASE 1: variante 1, q1 (barata) ----------------
    v1 = variants[0]

    q1 = build_q1(v1)
    stopped = eval_results(fetch_query(q1, count=per_query_count_q1))
    if stopped:
        return stopped
    pace()

    # Si q1 ya dio señal alta (>=thr_run_q2), NO gastes q2. Deja REVISAR y corta.
    ...
    q2 = build_q2(v1, carrera, max_terms=6)
    stopped = eval_results(fetch_query(q2, count=per_query_count_q2))
    if stopped:
        return stopped
    pace()

    # Si ya llegó a >=65 tras variante 1, corta y deja REVISAR
    ...
    for nv in variants[1:]:
        ...
        # solo usar q2 si q1 de esa variante tampoco dio señal
        if not best or best[0] < thr_run_q2:
            q2 = build_q2(nv, carrera, max_terms=6)
            stopped = eval_results(fetch_query(q2, count=per_query_count_q2))
            if stopped:
                return stopped
            pace()
...
def run_lote(
    ...
    for i, row in df.iterrows():
        ...
        # REINICIO SEGURO: si ya tiene URL o ya fue marcado SR, no re-procesa (no re-gasta)
